...
for angle in [90, 157.5]:
    ...
    with open(Path().home() / "Google Drive/Studium/Cambridge/thesis/TexContents/Figures/colours.json") as j:
        file = json.load(j)
        rgbC = file["crgb_list"][1] if angle < 100 else file["crgb_list"][0]
        rgbC = tuple(rgbC[:3])
    ...
    ax.plot([rX[0], rX[0]+length], [rY[0], rY[0]],
            [rZ[0], rZ[0]], color='black', lw=0.7)
    ax.text(rX[0]+length + offset, rY[0], rZ[0], r"$x$", va='center')
    ax.plot([rX[0], rX[0]], [rY[0], rY[0]+length],
            [rZ[0], rZ[0]], color='black', lw=0.7)
    ax.text(rX[0], rY[0]+length + offset, rZ[0],
            r"$y$", va='bottom', ha='left')
    ax.plot([rX[0], rX[0]], [rY[0], rY[0]], [
            rZ[0], rZ[0]+length], color='black', lw=0.7)
    ...
    # ax.grid(False)
    ax.set_axis_off()
    ax.view_init(azim=-67, elev=30)
    ax.dist = 4 if angle > 100 else 7.6
