...
Fig, axes = plt.subplots(2, 3, figsize=(
    6.5, 3), sharey='row', gridspec_kw=gs_kw)  # og fig height=3
...
axes[1, 0].scatter(xplot, yplot, color=c1, alpha=0.7,
                   s=45, marker='.', edgecolors='none')

axes[1, 1].plot(xTrap, yTrap, color='black')
axes[1, 1].plot(xTrap, np.ma.masked_where(
    yTrap <= 0.7, yTrap), color=(1, 1, 1), alpha=0.78)
axes[1, 1].plot(xTrap, 0.7 * np.ones(len(xTrap)),
                linestyle=':', color='black', alpha=0.9)
axes[1, 1].text(1.03, 0.7, r"$U'$", va='center')
xNew = xplot[yplot <= 0.7]
yNew = yplot[yplot <= 0.7]
axes[1, 1].scatter(xNew, yNew, color=c1, alpha=0.7,
                   s=45, marker='.', edgecolors='none')
xNew = xplot[yplot > 0.7]
yNew = yplot[yplot > 0.7]
axes[1, 1].scatter(xNew, yNew, color=c1, alpha=0.3,
                   s=45, marker='.', edgecolors='none')
...
axes[1, 2].scatter(xplot, yplot, color=c1, alpha=0.2,
                   s=30, marker='.', edgecolors='none')
axes[1, 2].scatter(xNew, yNew, color=c2, alpha=0.7,
                   s=45, marker='.', edgecolors='none')
...
for i in range(3):
    ...
    axes[0, i].axis("off")
    axes[1, i].axis("off")
    axes[0, i].text(-0.03, 1, r'$n(v)$', transform=axes[0,
                                                        i].transAxes, ha='right', va='center')
    axes[0, i].text(1.05, -0.03, r'$v$', transform=axes[0,
                                                        i].transAxes, ha='center', va='top')
    axes[1, i].text(-0.03, 1, r'$U$', transform=axes[1,
                                                     i].transAxes, ha='right', va='center')
...
path = Path().home() / \
    "Google Drive/Studium/Cambridge/thesis/TexContents/Figures/Evap/"
Fig.savefig(path / 'EvaporationSketch.pgf',
            bbox_inches='tight', pad_inches=0.02)
latexplot('EvaporationSketch', keeppgf=True)
